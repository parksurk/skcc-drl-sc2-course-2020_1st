...
class DQN(nn.Module):
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        with torch.no_grad():
            ...
            q_max_temp, _ = self.qnet_target(ns).max(dim=-1, keepdim=True)
            q_max_without_keepdim, _ = self.qnet_target(ns).max(dim=-1)
            q_target = r + self.gamma * q_max * (1 - done)

            print(q_max.shape)
            print(q_max_temp.shape)
            print(q_max_without_keepdim.shape)
